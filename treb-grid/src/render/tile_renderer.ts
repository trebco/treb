...
export class TileRenderer {
    ...
    for (const overflow of this.overflow_areas) {
      ...
      if (!dirty) {
        for (let column = overflow.area.start.column; !dirty && column <= overflow.area.end.column; column++) {
          const cell = this.model.sheet.cells.GetCell({ row, column }, false);
  ...
  public RenderCorner(/* selection: GridSelection */) {
    ...
    let x = header_size.x;
    for (let i = 0; i < this.layout.freeze.columns; i++) {
      x += this.model.sheet.ColumnWidth(i);
    }

    let y = header_size.y;
    for (let i = 0; i < this.layout.freeze.rows; i++) {
    ...
    if (this.layout.freeze.rows && this.model.sheet.header_offset.x > 1) {
      ...
      let row_index = 0;
      for (; row_index < this.layout.freeze.rows; row_index++) {
        ...
        {
          // if (row_index < this.layout.freeze.rows - 1) {
          context.moveTo(0, height - 0.5);
    ...
    if (this.layout.freeze.columns && this.model.sheet.header_offset.y > 1) {
      ...
      let column_index = 0;
      for (; column_index < this.layout.freeze.columns; column_index++) {
        ...
        {
          // if (column_index < this.layout.freeze.columns - 1) {
          context.moveTo(width - 0.5, 0);
  ...
  public RenderHeaders(tiles: TileRange /*, selection: GridSelection*/, force = false) {
    ...
    for (let column = tiles.start.column; column <= tiles.end.column; column++) {
      ...
      if (tile.dirty || force) {
        ...
        context.fillStyle = this.theme.header_background_color || '';
        context.fillRect(0, 0, tile.logical_size.width, this.model.sheet.header_offset.y);
    ...
    for (let row = tiles.start.row; row <= tiles.end.row; row++) {
      ...
      if (tile.dirty || force) {
        ...
        context.fillRect(0, 0, this.model.sheet.header_offset.x, tile.logical_size.height);
  ...
  public Render(tile: Tile) {
    ...
    if (tile.first_cell.row <= this.layout.freeze.rows - 1) {
      for (let i = tile.first_cell.row; i < this.layout.freeze.rows && i <= tile.last_cell.row; i++) {
        copy_height += this.model.sheet.RowHeight(i);
      }
    }
    if (tile.first_cell.column <= this.layout.freeze.columns - 1) {
      for (let i = tile.first_cell.column; i < this.layout.freeze.columns && i <= tile.last_cell.column; i++) {
    ...
    if (copy_width && copy_height) {
      ...
      corner_context.setTransform(scale, 0, 0, scale,
        this.model.sheet.header_offset.x * scale,
        this.model.sheet.header_offset.y * scale);
  ...
  protected RenderCellBorders(
    ...
    if (!composite.border_bottom) {
      const test = this.model.sheet.CellStyleData({ row: address.row + 1, column: address.column });
    ...
    if (!composite.border_left && address.column) {
      const test = this.model.sheet.CellStyleData({ row: address.row, column: address.column - 1 });
    ...
    if (cell.merge_area) {
      ...
      if ((address.row === cell.merge_area.start.row) &&
        ...
        if (cell.merge_area.count > 1) {
          const end_cell_style = this.model.sheet.CellStyleData(cell.merge_area.end);
          if (end_cell_style) {
